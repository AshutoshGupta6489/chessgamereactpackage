...
const ChessBoard = ({ boardSide = "w", setBoardData = [
    ['r', 'n', 'b', 'q', 'k', 'b', 'n', 'r'],
    ['p', 'p', 'p', 'p', 'p', 'p', 'p', 'p'],
    ['', '', '', '', '', '', '', ''],
    ['', '', '', '', '', '', '', ''],
    ...
    const [selectedPos, setSelectedPos] = React.useState(["", -1, -1]);
    const [starting, setStarting] = React.useState("");
    const validPosList = React.useRef();
    const underAttack = React.useRef();
    const resetPostion = React.useRef();
    const black = { 'P': `♙`, 'R': `♖`, 'N': `♘`, 'B': `♗`, 'Q': `♕`, 'K': `♔` };
    const white = { 'p': `♟`, 'r': `♜`, 'n': `♞`, 'b': `♝`, 'q': `♛`, 'k': `♚` };
    const [board, setBoard] = React.useState(setBoardData);
    React.useEffect(() => {
